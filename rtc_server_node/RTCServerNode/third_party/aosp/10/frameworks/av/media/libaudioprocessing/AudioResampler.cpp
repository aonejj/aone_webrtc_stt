...
#if defined(__ANDROID__)		// kimi... add
...
private:
    ...
    int mX0L;	// kimi ... 남아 있는것
    int mX0R;	// kimi ... 남아 있는것
};
...
#else 
	defaultQuality = LOW_QUALITY;		//kimi temp setting 
#endif
}
...
size_t AudioResamplerOrder1::resampleStereo16(int32_t* out, size_t outFrameCount,
    ...
    size_t outputSampleCount = outFrameCount * 2;			// kimi.... why outSampleCount가 outFrameCount * 2 ???, outSampleCount의 의미는???---> resample 결과
    size_t inFrameCount = getInFrameCountRequired(outFrameCount);

//     ALOGE("starting resample %d frames, inputIndex=%d, phaseFraction=%d, phaseIncrement=%d mBuffer.frameCount %d outputSampleCount %d",
//          outFrameCount, inputIndex, phaseFraction, phaseIncrement, mBuffer.frameCount, outputSampleCount);
...
//			ALOGE("boundary case inputIndex %d pharseFraction %d outputIndex %d", inputIndex, phaseFraction, outputIndex);
...
//		ALOGE("loop done - outputIndex=%d, inputIndex=%d mBuffer.frameCount %d", outputIndex, inputIndex, mBuffer.frameCount);
