...
#if defined(__ANDROID__)	// kimi disable
...
#if defined(__ANDROID__)		// kimi disable... can't include because so many relate files
        uint32_t type;
        const void *data;
        size_t size;
...
const uint8_t *findNextNalStartCode_3Bytes(const uint8_t *data, size_t length) {
	uint8_t *res = NULL;
	if (length > 3) {
		// minus 1 as to not match NAL start code at end
...
#if defined(__ANDROID__)	// kimi... disable for android
...
#define CHANNEL_MASK_USE_CHANNEL_ORDER 0	// kimi... add
...
#if defined(__ANDROID__)	// kimi... disable... dependency hardward...
    // Check if offload is possible for given format, stream type, sample rate,
    // bit rate, duration, video and streaming
    return AudioSystem::isOffloadSupported(info);
#else
	return true;	// kimi... thinking... always true setting... ummm...
#endif
}
